...
Read in the data and then remove all the missing values and add a new column that changes the date column from a factor class to a time (POSIXct and POSIXt) class and save this new data frame under the variable **data1**
...
The **mean** of the total steps taken each day is **`r total_steps_mean`** (rounded to the nearest whole number) and the **median** of the total steps taken each day is **`r total_steps_median`**.
...
On average across all days, the 5 minute interval that contains the **maximum number of steps** is the **`r max_steps_time$interval`th** interval
...
There are a total of **`r totalNA`** missing values (NA's) in the original dataset. 
...
The **mean** of the total steps taken each day is **`r total_steps_mean_all_data`** (rounded to the nearest whole number) and the **median** of the total steps taken each day is **`r total_steps_median_all_data`** (rounded to the nearest whole number). These values are basically similar to the values estimated from the first part of the assignment. By imputing the missing data, the total daily number of steps do not have any significant difference from the original estimates and this is because I added in the average data for the missing values based on the time interval and so by doing this I do not actually change the day-to-day values when looking at the whole time period at once. 
